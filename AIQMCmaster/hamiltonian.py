...
def local_kinetic_energy(f: nn.AINetLike) -> KineticEnergy:
    ...
    def _lapl_over_f(params, data):
        ...
        second_grad_value = jax.vmap(jax.jacfwd(jax.jacrev(logabs_f, argnums=1), argnums=1), in_axes=(None, 1, None, None),
                                     out_axes=0)
        hessian_value_logabs = second_grad_value(params, data.positions, data.atoms, data.charges)
        angle_grad_hessian = jax.vmap(jax.jacfwd(jax.jacrev(angle_f, argnums=1), argnums=1), in_axes=(None, 1, None, None),
                                      out_axes=0)
        ...
        angle_grad_value = jax.vmap(jax.grad(angle_f, argnums=1), in_axes=(None, 1, None, None), out_axes=0)
...
def local_energy(f: nn.AINetLike) -> LocalEnergy:
    ...
    Because the oprimizer dont accept the extra input, i.e. batch_atoms and batch_charges. We have to remove them.
    """
    ...
    def _e_l(batch_size: int, ndim: int, params: nn.ParamTree, data: nn.AINetData) -> jnp.array:
        ...
        batch_atoms = jnp.reshape(jnp.repeat(data.atoms, repeats=batch_size, axis=0), (1, batch_size, 2, 3))
        #jax.debug.print("batch_atoms:{}", batch_atoms)
        batch_charges = jnp.reshape(jnp.repeat(data.charges, repeats=batch_size, axis=0), (1, batch_size, 2))
        ...
        atoms_position_temp = jnp.reshape(batch_atoms, (batch_size, 2, ndim))  # 2 is the number of atoms.
        ...
        potential_E_ae = potential_electron_nuclear(r_ae, batch_charges, batch_size=batch_size)
        potential_E_aa = potential_nuclear_nuclear(batch_atoms, batch_charges, batch_size=batch_size)
...
#output = _e_l(batch_size=4, ndim=3, params=batch_params, data=data_non_batch)