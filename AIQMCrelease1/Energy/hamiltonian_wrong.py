...
from AIQMCrelease1.wavefunction_debug.nn_wrong import construct_input_features
import chex
from jax import lax
import kfac_jax
...
class MakeLocalEnergy(Protocol):
    def __call__(self, f: nn_wrong.AINetLike, **kwargs: Any) -> LocalEnergy:
...
def local_kinetic_energy(f: nn.AINetLike) -> KineticEnergy:
    ...
    def _lapl_over_f(params, data):
        ...
        value_angle, first_derivative_angle = grad_angle(params, data.positions, data.spins, data.atoms, data.charges)
        value_f, first_derivative_f = grad_f(params, data.positions, data.spins, data.atoms, data.charges)
...
def local_energy(signed_network: nn_wrong.AINetLike,
                 natoms: int,
                 nelectrons: int,
