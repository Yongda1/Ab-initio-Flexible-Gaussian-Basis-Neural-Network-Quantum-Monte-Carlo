...
import attr
import chex
from AIQMCrelease2.wavefunction import envelopes
from AIQMCrelease2.wavefunction import Jastrow
from AIQMCrelease2.wavefunction import network_blocks
import jax
...
def make_orbitals(
    ...
    def apply(
        ...
        """maybe it is working. now we need finish the other modules for this wavefunction.
        maybe we can consider add the isotropic envelope back later. 22.1.2025."""
        #envelope_factor = envelope.apply(ae, r_ae, xi=params['envelope']['xi'], natoms=natoms, nelectrons=nelectrons)
        #jax.debug.print("envelope_factor:{}", envelope_factor)
        #h_to_orbitals = h_to_orbitals * envelope_factor
        ...
        """here, we just import the electron-electron jastrow factors."""
...
atoms = jnp.array([[1.0, 1.0, 1.0], [2.0, 2.0, 2.0]])
charges = jnp.array([1.0, 1.0])
spins = jnp.array([1, -1])
