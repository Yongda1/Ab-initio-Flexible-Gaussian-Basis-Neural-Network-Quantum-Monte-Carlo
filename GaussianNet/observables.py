...
from GaussianNet import density
...
def make_observable_fns(fns, pmap: bool = True):
  ...
  def transform(fn):
    data_axes = networks.GaussianNetData(positions=0, spins=0, atoms=0, charges=0)
...
def make_s2(
    signed_network: networks.GaussianNetLike,
    nspins: Tuple[int, ...],
...
def make_density_matrix(
  ...
  def density_update(key,
    ...
    for _ in range(1000 if state.t == 0 else 1):  # burn-in on the first step
      ...
      rprime_data = networks.GaussianNetData(
