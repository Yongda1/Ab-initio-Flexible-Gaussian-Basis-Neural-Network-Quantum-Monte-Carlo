...
from GaussianNet.pseudopotential import pp_energy_test
from GaussianNet.pseudopotential import pseudopotential
...
class MakeLocalEnergy(Protocol):
    ...
    def __call__(
            self,
            f: nn.GaussianNetData,
            lognetwork: nn.LogGaussianNetLike,
            charges: jnp.ndarray,
            nspins: Sequence[int],
            use_scan: bool = False,
...
KineticEnergy = Callable[[nn.ParamTree, nn.GaussianNetData], jnp.ndarray]
...
def local_energy(
    ...
    def _e_l(params: nn.ParamTree,
             key: chex.PRNGKey,
             data: nn.GaussianNetData) -> Tuple[jnp.ndarray, Optional[jnp.ndarray]]:
