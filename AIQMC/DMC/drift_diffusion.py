...
def propose_drift_diffusion_new(f: networks.FermiNetLike,
    ...
    def drift_diffusion_new(params, key: chex.PRNGKey, data: networks.FermiNetData):
        ...
        #O_old_eff = jax.vmap(limdrift, in_axes=(0, None, None))(O_old, tstep, 0.25)
        ...
        for i in range(len(x1)):
            #jax.debug.print("x1[i]:{}", x1[i])
            gauss = jnp.sqrt(tstep) * jax.random.normal(key=key, shape=(jnp.shape(x1[i])))
            ...
            #jax.debug.print("wave_x1_mag:{}", wave_x1_mag)
            #jax.debug.print("wave_x2_mag:{}", wave_x2_mag)
