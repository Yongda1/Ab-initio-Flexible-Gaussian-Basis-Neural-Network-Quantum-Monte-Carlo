...
from AIQMCpretrain1.wavefunction import hamiltonian
from AIQMCpretrain1.wavefunction import networks
#import folx
import jax
...
def make_loss(network: networks.LogAINetLike,
  ...
  batch_local_energy = vmap(
      ...
      in_axes=(
          None,
          0,
          networks.AINetData(positions=0, spins=0, atoms=0, charges=0),
      ),
      out_axes=(0, 0)
  )
...
def make_wqmc_loss(
  ...
  def total_energy(
    ...
    def batch_local_energy_pos(pos):
      network_data = networks.AINetData(
          positions=pos,
          spins=data.spins,
          atoms=data.atoms,
...
def make_energy_overlap_loss(network: networks.LogAINetLike,
                             local_energy: hamiltonian.LocalEnergy,
                             clip_local_energy: float = 0.0,
                             clip_from_median: bool = True,
                             center_at_clipped_energy: bool = True,
  ...
  vmap = jax.vmap if max_vmap_batch_size == 0 else functools.partial(
      folx.batched_vmap, max_batch_size=max_vmap_batch_size)
  data_axes = networks.AINetData(positions=0, spins=0, atoms=0, charges=0)
  batch_local_energy = vmap(
