...
KineticEnergy = Callable[
    [networks.ParamTree, networks.AINetData], jnp.ndarray
]
...
def local_energy(
  ...
  def _e_l(
      params: networks.ParamTree, key: chex.PRNGKey, data: networks.AINetData
  ) -> Tuple[jnp.ndarray, Optional[jnp.ndarray]]:
    ...
    if states:
      ...
      if use_pp:
        data_vmap_dims = networks.AINetData(
            positions=0, spins=0, atoms=None, charges=None)
        data_ = networks.AINetData(
            positions=positions,
