...
class RepeatedDenseBlock(kfac_jax.DenseTwoKroneckerFactored):
    ...
    def update_curvature_matrix_estimate(
        ...
        return super().update_curvature_matrix_estimate(state=state, estimation_data=estimation_data, ema_old=ema_old,
                                                        ema_new=ema_new, batch_size=batch_size)
...
class QmcBlockedDense(kfac_jax.TwoKroneckerFactored):
    ...
    def _init(self, rng: kfac_jax.utils.PRNGKey, exact_powers_to_cache: Set[Scalar],
              approx_powers_to_cache: Set[Scalar], cache_eigenvalues: bool, ) -> kfac_jax.TwoKroneckerFactored.State:
        ...
        return kfac_jax.TwoKroneckerFactored.State(cache=cache,
                                                   input_factor=kfac_jax.utils.WeightedMovingAverage.zeros_array(
                                                       (j, k, k)),
    ...
    def _update_cache(self, state: kfac_jax.TwoKroneckerFactored.State, identity_weight: kfac_jax.utils.Numeric,
        ...
        for power in approx_powers:
            ...
            cache["inputs_factor"], cache["outputs_factor"] = pi_adjusted_inverse(state.inputs_factor.value,
...
repeated_dense2_with_bias_pattern = kfac_jax.tag_graph_matcher.GraphPattern(name="repeated_dense2_with_bias",
                                                                            ...
                                                                            parameters_extractor_func=_dense_parameter_extractor,
                                                                            example_args=[np.zeros([8, 9, 11, 13]),
                                                                                          [np.zeros([13, 7]),
                                                                                           np.zeros([7])]])
